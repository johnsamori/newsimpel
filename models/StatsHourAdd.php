 ...
 * Page class
 */
class StatsHourAdd extends StatsHour
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StatsHourAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "statshouradd";
...
// Is modal
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("Hour") ?? Route("Hour")) !== null) {
                $this->Hour->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("statshourlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "statshourlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "statshourview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "statshourlist") {
                            ...
                            $returnUrl = "statshourlist"; // Return list page content
                        }
                    }
...
// Get upload files
    ...
    {
        $this->Year->DefaultValue = $this->Year->getDefault(); // PHP
        $this->Year->OldValue = $this->Year->DefaultValue;
        $this->Month->DefaultValue = $this->Month->getDefault(); // PHP
        $this->Month->OldValue = $this->Month->DefaultValue;
        $this->Date->DefaultValue = $this->Date->getDefault(); // PHP
        $this->Date->OldValue = $this->Date->DefaultValue;
        $this->Hour->DefaultValue = $this->Hour->getDefault(); // PHP
        $this->Hour->OldValue = $this->Hour->DefaultValue;
        $this->Hits->DefaultValue = $this->Hits->getDefault(); // PHP
        $this->Hits->OldValue = $this->Hits->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Hour' first before field var 'x_Hour'
        $val = $CurrentForm->hasValue("Hour") ? $CurrentForm->getValue("Hour") : $CurrentForm->getValue("x_Hour");
        if (!$this->Hour->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Hour->Visible = false; // Disable update for API request
            } else {
                $this->Hour->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // Hour
        $this->Hour->RowCssClass = $this->IsMobileOrModal ? "row" : "";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (strval($this->Year->EditValue) != "" && is_numeric($this->Year->EditValue)) {
                $this->Year->EditValue = $this->Year->EditValue;
            }
            ...
            if (strval($this->Date->EditValue) != "" && is_numeric($this->Date->EditValue)) {
                $this->Date->EditValue = FormatNumber($this->Date->EditValue, null);
            }

            // Hour
            $this->Hour->setupEditAttributes();
            $this->Hour->EditValue = HtmlEncode($this->Hour->CurrentValue);
            $this->Hour->PlaceHolder = RemoveHtml($this->Hour->caption());
            if (strval($this->Hour->EditValue) != "" && is_numeric($this->Hour->EditValue)) {
                $this->Hour->EditValue = FormatNumber($this->Hour->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->Hour->Required) {
            if (!$this->Hour->IsDetailKey && EmptyValue($this->Hour->FormValue)) {
                $this->Hour->addErrorMessage(str_replace("%s", $this->Hour->caption(), $this->Hour->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->Hour->FormValue)) {
            $this->Hour->addErrorMessage($this->Hour->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Year
        $this->Year->setDbValueDef($rsnew, $this->Year->CurrentValue, strval($this->Year->CurrentValue) == "");

        // Month
        $this->Month->setDbValueDef($rsnew, $this->Month->CurrentValue, strval($this->Month->CurrentValue) == "");

        // Date
        $this->Date->setDbValueDef($rsnew, $this->Date->CurrentValue, strval($this->Date->CurrentValue) == "");

        // Hour
        $this->Hour->setDbValueDef($rsnew, $this->Hour->CurrentValue, strval($this->Hour->CurrentValue) == "");

        // Hits
        $this->Hits->setDbValueDef($rsnew, $this->Hits->CurrentValue, strval($this->Hits->CurrentValue) == "");
        ...
        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['Date']) == "") {
            $this->setFailureMessage($Language->phrase("InvalidKeyValue"));
            $insertRow = false;
        }

        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['Hour']) == "") {
            $this->setFailureMessage($Language->phrase("InvalidKeyValue"));
            $insertRow = false;
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("statshourlist"), "", $this->TableVar, true);
