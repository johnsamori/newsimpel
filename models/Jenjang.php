...
/**
 * Table class for jenjang
 */
class Jenjang extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Nama_Jenjang
        $this->Nama_Jenjang = new DbField(
            $this, // Table
            'x_Nama_Jenjang', // Variable name
            'Nama_Jenjang', // Name
            '`Nama_Jenjang`', // Expression
            '`Nama_Jenjang`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nama_Jenjang`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Nama_Jenjang->InputTextType = "text";
        $this->Nama_Jenjang->Nullable = false; // NOT NULL field
        $this->Nama_Jenjang->Required = true; // Required field
        $this->Nama_Jenjang->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['Nama_Jenjang'] = &$this->Nama_Jenjang;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "jenjang";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Nama_Jenjang->DbValue = $row['Nama_Jenjang'];
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->no->OldValue) && !$current ? $this->no->OldValue : $this->no->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("jenjanglist");
    }
    ...
    {
        global $Language;
        if ($pageName == "jenjangview") {
            return $Language->phrase("View");
        } elseif ($pageName == "jenjangedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "jenjangadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("jenjangview", $parm);
        } else {
            $url = $this->keyUrl("jenjangview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jenjangedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jenjanglist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jenjangadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("jenjanglist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("jenjangdelete");
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->Nama_Jenjang->EditValue = $this->Nama_Jenjang->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Nama_Jenjang);
                } else {
                    $doc->exportCaption($this->no);
                    $doc->exportCaption($this->Nama_Jenjang);
                }
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Nama_Jenjang);
                    } else {
                        $doc->exportField($this->no);
                        $doc->exportField($this->Nama_Jenjang);
                    }
