 ...
 * Page class
 */
class StatsHourEdit extends StatsHour
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StatsHourEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "statshouredit";
...
// Is modal
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("Hour") ?? Key(3) ?? Route(5)) !== null) {
                $this->Hour->setQueryStringValue($keyValue);
                $this->Hour->setOldValue($this->Hour->QueryStringValue);
            } elseif (Post("Hour") !== null) {
                $this->Hour->setFormValue(Post("Hour"));
                $this->Hour->setOldValue($this->Hour->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("statshourlist"); // Return to list page
                        return;
                        ...
                        if ($this->Year->CurrentValue != null && $this->Month->CurrentValue != null && $this->Date->CurrentValue != null && $this->Hour->CurrentValue != null) {
                            while (!$rs->EOF) {
                                if (SameString($this->Year->CurrentValue, $rs->fields['Year']) && SameString($this->Month->CurrentValue, $rs->fields['Month']) && SameString($this->Date->CurrentValue, $rs->fields['Date']) && SameString($this->Hour->CurrentValue, $rs->fields['Hour'])) {
...
// Get upload files
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_Hour")) {
            $this->Hour->setOldValue($CurrentForm->getValue("o_Hour"));
        }
    ...
    protected function editRow()
    {
        ...
        // Hour
        $this->Hour->setDbValueDef($rsnew, $this->Hour->CurrentValue, $this->Hour->ReadOnly);
