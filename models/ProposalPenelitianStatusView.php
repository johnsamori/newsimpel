 ...
 * Page class
 */
class ProposalPenelitianStatusView extends ProposalPenelitianStatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProposalPenelitianStatusView";
    ...
    // CSS class/style
    public $CurrentPageName = "proposalpenelitianstatusview";
    ...
    {
        $this->id_pengabidian->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Table object (proposal_penelitian_status)
        if (!isset($GLOBALS["proposal_penelitian_status"]) || get_class($GLOBALS["proposal_penelitian_status"]) == PROJECT_NAMESPACE . "proposal_penelitian_status") {
            $GLOBALS["proposal_penelitian_status"] = &$this;
        }

        // Set up record key
        if (($keyValue = Get("id_pengabidian") ?? Route("id_pengabidian")) !== null) {
            $this->RecKey["id_pengabidian"] = $keyValue;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'proposal_penelitian_status');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "proposalpenelitianstatusview"; // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->id_pengabidian->Visible = false;
        }
    }
...
// Is modal
        ...
        if (($keyValue = Get("id_pengabidian") ?? Route("id_pengabidian")) !== null) {
            $this->id_pengabidian->setQueryStringValue($keyValue);
            $this->RecKey["id_pengabidian"] = $this->id_pengabidian->QueryStringValue;
        } elseif (Post("id_pengabidian") !== null) {
            $this->id_pengabidian->setFormValue(Post("id_pengabidian"));
            $this->RecKey["id_pengabidian"] = $this->id_pengabidian->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_pengabidian->setQueryStringValue($keyValue);
            $this->RecKey["id_pengabidian"] = $this->id_pengabidian->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "proposalpenelitianstatuslist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("proposalpenelitianstatuslist"); // Return to list page
                        return;
                        ...
                        while (!$this->Recordset->EOF) {
                            if (SameString($this->id_pengabidian->CurrentValue, $this->Recordset->fields['id_pengabidian'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "proposalpenelitianstatuslist"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        if ($this->IsModal) {
			$item->Body = "<a class=\"ew-action ew-add\" title=\"" . $addcaption . "\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-ask=\"1\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\">" . $Language->phrase("ViewPageAddLink") . "</a>";
        } else {
        ...
        if ($this->IsModal) {
			$item->Body = "<a class=\"ew-action ew-edit\" title=\"" . $editcaption . "\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-ask=\"1\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\">" . $Language->phrase("ViewPageEditLink") . "</a>";
        } else {
...
// Load recordset
    ...
    {
        $row = [];
        $row['id_pengabidian'] = $this->id_pengabidian->DefaultValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fproposal_penelitian_statusview\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fproposal_penelitian_statusview\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fproposal_penelitian_statusview\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fproposal_penelitian_statusview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . HtmlEncode(ArrayToJsonAttribute($this->RecKey)) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("proposalpenelitianstatuslist"), "", $this->TableVar, true);
        $pageId = "view";
