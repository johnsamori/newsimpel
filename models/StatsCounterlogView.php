 ...
 * Page class
 */
class StatsCounterlogView extends StatsCounterlog
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StatsCounterlogView";
    ...
    // CSS class/style
    public $CurrentPageName = "statscounterlogview";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("IP_Address") ?? Route("IP_Address")) !== null) {
            $this->RecKey["IP_Address"] = $keyValue;
        }
...
// Is modal
        ...
        } elseif (Post("IP_Address") !== null) {
            $this->IP_Address->setFormValue(Post("IP_Address"));
            $this->RecKey["IP_Address"] = $this->IP_Address->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->IP_Address->setQueryStringValue($keyValue);
            $this->RecKey["IP_Address"] = $this->IP_Address->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "statscounterloglist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("statscounterloglist"); // Return to list page
                        return;
                        ...
                        while (!$this->Recordset->EOF) {
                            if (SameString($this->IP_Address->CurrentValue, $this->Recordset->fields['IP_Address'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "statscounterloglist"; // No matching record, return to list
                    } else {
...
// Load recordset
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fstats_counterlogview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . HtmlEncode(ArrayToJsonAttribute($this->RecKey)) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        $rs = null;
        if (count($keys) >= 1) {
            $this->IP_Address->OldValue = $keys[0];
