 ...
 * Page class
 */
class ProposalPengabdianStatusList extends ProposalPengabdianStatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProposalPengabdianStatusList";
    ...
    public $FormName = "fproposal_pengabdian_statuslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "proposalpengabdianstatuslist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'proposal_pengabdian_status';
        $this->TableName = 'proposal_pengabdian_status';
        ...
        // Table object (proposal_pengabdian_status)
        if (!isset($GLOBALS["proposal_pengabdian_status"]) || get_class($GLOBALS["proposal_pengabdian_status"]) == PROJECT_NAMESPACE . "proposal_pengabdian_status") {
            $GLOBALS["proposal_pengabdian_status"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "proposalpengabdianstatusadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "proposalpengabdianstatusdelete";
        $this->MultiUpdateUrl = "proposalpengabdianstatusupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'proposal_pengabdian_status');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "proposalpengabdianstatusview"; // If View page, no primary button
                } else { // List page
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fproposal_pengabdian_statusgrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("proposalpengabdianstatuslist");
                    return;
                }
            } else {
		...
		if (@$_COOKIE['proposal_pengabdian_status_searchpanel'] == 'notactive') {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (@$_COOKIE['proposal_pengabdian_status_searchpanel'] == 'active') {
    ...
    protected function setupDisplayRecords()
    {
		...
		if ($wrk > 50 || strtolower($wrk) == "all") {
            $wrk = 50;
            $this->setFailureMessage(str_replace("%t", 50, $Language->Phrase("MaximumRecordsPerPage")));
        }
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_pengabdian->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_Kelompok_ID") && $CurrentForm->hasValue("o_Kelompok_ID") && $this->Kelompok_ID->CurrentValue != $this->Kelompok_ID->DefaultValue) {
            return false;
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fproposal_pengabdian_statussrch");
        }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fproposal_pengabdian_statussrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_pengabdian); // id_pengabdian
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"proposal_pengabdian_status\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"proposal_pengabdian_status\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproposal_pengabdian_statuslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproposal_pengabdian_statuslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_pengabdian->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"proposal_pengabdian_status\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fproposal_pengabdian_statuslist\"" .
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-edit\" title=\"" .
            $Language->phrase("EditSelectedLink", true) . "\" data-table=\"proposal_pengabdian_status\" data-caption=\"" .
            $Language->phrase("EditSelectedLink", true) . "\" form=\"fproposal_pengabdian_statuslist\" data-ew-action=\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("id_pengabdian", $this->createColumnOption("id_pengabdian"));
            $option->add("Kelompok_ID", $this->createColumnOption("Kelompok_ID"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fproposal_pengabdian_statussrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fproposal_pengabdian_statussrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fproposal_pengabdian_statuslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fproposal_pengabdian_statuslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fproposal_pengabdian_statuslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_proposal_pengabdian_status", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_proposal_pengabdian_status",
...
// Load default values
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_pengabdian->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_Kelompok_ID")) {
            $this->Kelompok_ID->setOldValue($CurrentForm->getValue("o_Kelompok_ID"));
        }
    ...
    {
        $row = [];
        $row['id_pengabdian'] = $this->id_pengabdian->DefaultValue;
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->Kelompok_ID->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter("`Id_kelompok`", "=", $this->Kelompok_ID->CurrentValue, DATATYPE_STRING, "");
                }
            ...
            // id_pengabdian
            $this->id_pengabdian->setupEditAttributes();
    ...
    protected function validateForm()
    {
        ...
        if ($this->id_pengabdian->Required) {
            if (!$this->id_pengabdian->IsDetailKey && EmptyValue($this->id_pengabdian->FormValue)) {
                $this->id_pengabdian->addErrorMessage(str_replace("%s", $this->id_pengabdian->caption(), $this->id_pengabdian->RequiredErrorMessage));
            }
        }
        if ($this->Kelompok_ID->Required) {
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (Kelompok_ID)
        if ($this->Kelompok_ID->CurrentValue != "") {
            $filterChk = "(`Kelompok_ID` = '" . AdjustSql($this->Kelompok_ID->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->Kelompok_ID->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->Kelompok_ID->CurrentValue, $idxErrMsg);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['Kelompok_ID']); // Kelompok_ID
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Update current values
        $this->setCurrentValues($rsnew);
        if ($this->Kelompok_ID->CurrentValue != "") { // Check field with unique index
            $filter = "(`Kelompok_ID` = '" . AdjustSql($this->Kelompok_ID->CurrentValue, $this->Dbid) . "')";
    ...
    protected function setupSearchOptions()
    {
		...
		if (@$_COOKIE['proposal_pengabdian_status_searchpanel'] == 'notactive') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fproposal_pengabdian_statussrch\" aria-pressed=\"false\">" . $Language->phrase("SearchLink") . "</a>";
		} else {
			// $item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fproposal_pengabdian_statussrch\" aria-pressed=\"false\">" . $Language->phrase("SearchLink") . "</a>";
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fproposal_pengabdian_statussrch\" aria-pressed=\"true\">" . $Language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
