 ...
 */
class Breadcrumblinks extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->Page_Title->IsPrimaryKey = true; // Primary key field
        ...
        // Lft
        $this->Lft = new DbField(
            $this, // Table
            'x_Lft', // Variable name
            'Lft', // Name
            '`Lft`', // Expression
            '`Lft`', // Basic search expression
            3, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Lft`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Lft->InputTextType = "text";
        $this->Lft->Nullable = false; // NOT NULL field
        $this->Lft->Required = true; // Required field
        $this->Lft->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Lft->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Lft'] = &$this->Lft;

        // Rgt
        $this->Rgt = new DbField(
            $this, // Table
            'x_Rgt', // Variable name
            'Rgt', // Name
            '`Rgt`', // Expression
            '`Rgt`', // Basic search expression
            3, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Rgt`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Rgt->InputTextType = "text";
        $this->Rgt->Nullable = false; // NOT NULL field
        $this->Rgt->Required = true; // Required field
        $this->Rgt->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Rgt->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "breadcrumblinks";
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Page_Title', $rs)) {
                AddFilter($where, QuotedName('Page_Title', $this->Dbid) . '=' . QuotedValue($rs['Page_Title'], $this->Page_Title->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->Page_Title->CurrentValue : $this->Page_Title->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Page_Title->CurrentValue = $keys[0];
            } else {
                $this->Page_Title->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Page_Title', $row) ? $row['Page_Title'] : null;
        } else {
            $val = !EmptyValue($this->Page_Title->OldValue) && !$current ? $this->Page_Title->OldValue : $this->Page_Title->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@Page_Title@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("breadcrumblinkslist");
    }
    ...
    {
        global $Language;
        if ($pageName == "breadcrumblinksview") {
            return $Language->phrase("View");
        } elseif ($pageName == "breadcrumblinksedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "breadcrumblinksadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "BreadcrumblinksView";
            case Config("API_ADD_ACTION"):
                return "BreadcrumblinksAdd";
            case Config("API_EDIT_ACTION"):
                return "BreadcrumblinksEdit";
            case Config("API_DELETE_ACTION"):
                return "BreadcrumblinksDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("breadcrumblinksview", $parm);
        } else {
            $url = $this->keyUrl("breadcrumblinksview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "breadcrumblinksadd?" . $parm;
        } else {
            $url = "breadcrumblinksadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("breadcrumblinksedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("breadcrumblinkslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("breadcrumblinksadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("breadcrumblinkslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("breadcrumblinksdelete");
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Page_Title\":" . JsonEncode($this->Page_Title->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->Page_Title->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Page_Title->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("Page_Title") ?? Route("Page_Title")) !== null) {
                $arKeys[] = $keyValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Lft);
                    $doc->exportCaption($this->Rgt);
                } else {
                    $doc->exportCaption($this->Page_Title);
                    $doc->exportCaption($this->Page_URL);
                    $doc->exportCaption($this->Lft);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Lft);
                        $doc->exportField($this->Rgt);
                    } else {
                        $doc->exportField($this->Page_Title);
                        $doc->exportField($this->Page_URL);
                        $doc->exportField($this->Lft);
