...
/**
 * Table class for myuserprofile
 */
class Myuserprofile extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "myuserprofile";
        $this->Dbid = 'DB';
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->Activated->Lookup = new Lookup('Activated', 'myuserprofile', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
                break;
            case "id-ID":
                $this->Activated->Lookup = new Lookup('Activated', 'myuserprofile', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->Locked->Lookup = new Lookup('Locked', 'myuserprofile', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
                break;
            case "id-ID":
                $this->Locked->Lookup = new Lookup('Locked', 'myuserprofile', false, '', ["","","",""], '', '', [], [], [], [], [], [], '', '', "");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "myuserprofile";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("myuserprofilelist");
    }
    ...
    {
        global $Language;
        if ($pageName == "myuserprofileview") {
            return $Language->phrase("View");
        } elseif ($pageName == "myuserprofileedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "myuserprofileadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "MyuserprofileView";
            case Config("API_ADD_ACTION"):
                return "MyuserprofileAdd";
            case Config("API_EDIT_ACTION"):
                return "MyuserprofileEdit";
            case Config("API_DELETE_ACTION"):
                return "MyuserprofileDelete";
            case Config("API_LIST_ACTION"):
                return "MyuserprofileList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("myuserprofileview", $parm);
        } else {
            $url = $this->keyUrl("myuserprofileview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "myuserprofileadd?" . $parm;
        } else {
            $url = "myuserprofileadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("myuserprofileedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("myuserprofilelist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("myuserprofileadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("myuserprofilelist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("myuserprofiledelete");
        }
    }
    ...
    public function renderListRow()
    {
        ...
        if ($this->Photo->UseColorbox) {
            ...
            $this->Photo->LinkAttrs["data-rel"] = "myuserprofile_x_Photo";
    ...
    {
        if (!IsAdmin() && $rs["Username"] != CurrentUserName()) {
    		$this->setFailureMessage("Sorry, you cannot modify other user profile.");
    		$this->terminate("myuserprofilelist");
    	}
    }
    // Row Inserting event
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        if (!empty($rsnew["Photo"])) {  // pastikan ada file yang di-upload, updated on January 24, 2017
    		$date_upload = date("Y-m-d H:i:s");
    		$file_extension = substr(strtolower(strrchr($rsnew["Photo"], ".")), 1);
    		$date_upload = str_replace("-", "", $date_upload);
    		$date_upload = str_replace(":", "", $date_upload);
    		$date_upload = str_replace(" ", "", $date_upload);
    		$rsnew["Photo"] = $rsnew["Username"] . "_" . $date_upload . "." . $file_extension;
    	}
        return true;
    }
    ...
    {
        if (!empty($rsnew["New_Password"])) {
    		$rsnew["Password"] = EncryptPassword($rsnew["New_Password"]);
    	} else {
    		$rsnew["Password"] = $rsold["Password"];
    	}
        if (!empty($rsnew["Photo"])) { // pastikan ada file yang di-upload, updated on January 24, 2017
    		$date_upload = date("Y-m-d H:i:s");
    		$file_extension = substr(strtolower(strrchr($rsnew["Photo"], ".")), 1);
    		...
    		$date_upload = str_replace(":", "", $date_upload);
    		$date_upload = str_replace(" ", "", $date_upload);
    		$rsnew["Photo"] = $rsold["Username"] . "_" . $date_upload . "." . $file_extension;
    	}
        return true;
    }
    // Row Updated event
    ...
    {
        if (!empty($rsnew["New_Password"])) {
            $this->setSuccessMessage("Password is successfully changed.");
        }
    }
    ...
    public function rowRendered()
    {
        if (CurrentPageID() == "edit") {
        	$this->New_Password->EditValue = ""; // leave empty if do not want to change password
        }
    }
