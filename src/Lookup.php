    ...
    public function renderViewRow($row, $renderer = null)
    {
        ...
        foreach ($this->DisplayFields as $index => $name) {
            ...
            if ($displayField) {
                ...
                if (!EmptyString($viewValue) && !($sameTable && $name == $this->Name && !in_array($displayField->DataType, [DATATYPE_DATE, DATATYPE_TIME]) && $displayField->OptionCount == 0)) { // Make sure that ViewValue is not empty and not self lookup field (except Date/Time) and not field with user values
