...
class AdvancedSecurity
{
    ...
    public function canExportToPrint() {
        return (($this->CurrentUserLevel & ALLOW_PRINT) == ALLOW_PRINT);
    }

    public function setCanExportToPrint($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_PRINT;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_PRINT;
        }
    }
    ...
    public function canExportToHTML() {
        return (($this->CurrentUserLevel & ALLOW_HTML) == ALLOW_HTML);
    }

    public function setCanExportToHTML($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_HTML;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_HTML;
        }
    } 
    ...
    public function canExportToExcel() {
        return (($this->CurrentUserLevel & ALLOW_EXCEL) == ALLOW_EXCEL);
    }

    public function setCanExportToExcel($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_EXCEL;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_EXCEL;
        }
    }
    ...
    public function canExportToWord() {
        return (($this->CurrentUserLevel & ALLOW_WORD) == ALLOW_WORD);
    }

    public function setCanExportToWord($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_WORD;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_WORD;
        }
    }     
    ...
    public function canExportToPDF() {
        return (($this->CurrentUserLevel & ALLOW_PDF) == ALLOW_PDF);
    }

    public function setCanExportToPDF($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_PDF;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_PDF;
        }
    }     
    ...
    public function canExportToXML() {
        return (($this->CurrentUserLevel & ALLOW_XML) == ALLOW_XML);
    }

    public function setCanExportToXML($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_XML;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_XML;
        }
    }     
    ...
    public function canExportToCSV() {
        return (($this->CurrentUserLevel & ALLOW_CSV) == ALLOW_CSV);
    }

    public function setCanExportToCSV($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_CSV;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_CSV;
        }
    }     
    ...
    public function canExportToEmail() {
        return (($this->CurrentUserLevel & ALLOW_EMAIL) == ALLOW_EMAIL);
    }

    public function setCanExportToEmail($b) {
        if ($b) {
            $this->CurrentUserLevel |= ALLOW_EMAIL;
        } else {
            $this->CurrentUserLevel ^= ~ALLOW_EMAIL;
        }
    }     
	...
	// Check privilege for Delete page (for Detail-Delete)
    ...
    public function loadUserID()
    {
        ...
        if (strval($this->CurrentUserID) == "") {
            ...
            while ($userIDList != $curUserIDList) {
                $filter = GetMultiValueFilter('`Report_To`', $this->UserID,  Config("USER_TABLE_DBID"));
                $sql = $UserTable->getSql($filter);
                $rows = Conn($UserTable->Dbid)->executeQuery($sql)->fetchAll();
                foreach ($rows as $row) {
    ...
    {
        return implode(", ", array_map(fn($userId) => QuotedValue($userId, DATATYPE_STRING, Config("USER_TABLE_DBID")), $this->UserID));
    }
