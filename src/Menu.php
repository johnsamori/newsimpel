...
class Menu
{
    ...
    {
        $item = new MenuItem($id, $name, $text, $url, $parentId, $src, $allowed, $isHeader, $isCustomUrl, $icon, $label, $isNavbarItem, $isSidebarItem); // added $src by Masino Sinaga, July 17, 2022 to store table name
        ...
        // Set item active
        if (!$item->IsCustomUrl && CurrentPageName() == GetPageName($item->Url) || $item->IsCustomUrl && CurrentUrl() == GetUrl($item->Url) || $item->IsCustomUrl && CurrentPageName() == GetPageName($item->Url) || $item->Source == CurrentTableName()) { // Active
            $item->Active = true;
        }
    }
