...
function SessionTimeoutTime()
{
    ...
    return $mlt; // removed by Masino Sinaga --> - 30; // Add some safety margin
}
...
function MasinoBreadcrumbLinks() {
	...
	$sSql = "SELECT C.* FROM " . Config("MS_MASINO_BREADCRUMBLINKS_TABLE") . " AS B, " . Config("MS_MASINO_BREADCRUMBLINKS_TABLE") . " AS C WHERE (B.Lft BETWEEN C.Lft AND C.Rgt) AND (B.Page_URL LIKE '" . CurrentPageName() . "') ORDER BY C.Lft";
	...
	if (!empty($cnt)) {
		...
		if ($stmt->rowCount() > 0) {
			while ($row = $stmt->fetch()) {
				if ($i==1) {
					$nav .= "<li><a href='" . $basePath . "index' data-message='wait' title='".$Language->breadcrumbPhrase("Home")."' alt='".$Language->breadcrumbPhrase("Home")."' class='ew-home' /><span class='fa fa-home ew-icon'></span></a>&nbsp;<span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span>&nbsp;</li>";
				} else {
					if ($i >= $stmt->rowCount()) {
						$nav .= "<li class=\"active\"><strong>" . $Language->breadcrumbPhrase($row["Page_Title"]) . "</strong></li>";
					} else {
						if (MS_BREADCRUMBLINKS_NO_LINKS) {
							$nav .= "<li>".$Language->breadcrumbPhrase($row["Page_Title"])."&nbsp;<span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span>&nbsp;</li>";
						} else {
							if (empty(trim($row["Page_URL"]))) {
								$nav .= "<li>".$Language->breadcrumbPhrase($row["Page_Title"])."&nbsp;<span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span>&nbsp;</li>";
							} else {
								$nav .= "<li><a href='". $basePath . $row["Page_URL"] ."' data-message='wait'>".$Language->breadcrumbPhrase($row["Page_Title"])."</a>&nbsp;<span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span>&nbsp;</li>";
							}
						}
					}
				}
