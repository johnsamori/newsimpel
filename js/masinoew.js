/*!
 * JavaScript for PHPMaker v2023.10.0
...
(function (ew$1, $$1, luxon) {
  ...
  function FormBase(id, pageId) {
    ...
    this.setupFilters = function (e, filters) {
      ...
      $save.off("click.ew").on("click.ew", function (e) {
        ...
        ew.prompt({
          input: "text",
          title: ew.language.phrase("EnterFilterName")
        }, name => {
          name = ew.sanitize(name);
          if (name) {
            filters.push([name, data]);
            saveFilters(id, filters);
			alertify.success(ew.language.phrase('MessageAdded')).dismissOthers();
          }
        }, true);
      }).prevAll().remove();
      ...
      if (filters.length) {
        ...
        filters.forEach((filter, i, ar) => {
          $__default.default('<li><a class="dropdown-item" data-index="' + i + '" data-ew-action="none">' + filter[0] + '</a></li>').on("click", function (e) {
            ...
            ew.prompt({title: ew.language.phrase("DeleteFilterConfirm").replace("%s", filter[0]), icon: 'question'}, result => {
              if (result) {
                ar.splice(index, 1);
                saveFilters(id, filters);
				alertify.success(ew.language.phrase('MessageDeleted')).dismissOthers();
              }
            });
          }).appendTo($submenu);
      ...
      if (this.$element.is("form")) {
        ...
        $form.find("#btn-cancel[data-href]").on("click", function () {
          ...
          if (self.modified && ew.hasFormData(form)) {
            ew.prompt({title: ew.language.phrase("ConfirmCancel"), icon: 'question'}, result => {
              if (result) {
  ...
  let Form = /*#__PURE__*/function (_FormBase) {
    ...
    _proto.validate = async function validate(e) {
      ...
      if (this.pageId == "update" && !ew.updateSelected(form)) {
        ew.alert({title: ew.language.phrase("NoFieldSelected"), icon: "error"});
        return false;
      }
      let addcnt = 0,
      ...
      if (this.pageId == "list" && gridinsert && addcnt == 0) {
        // No row added
        ew.alert({
          title: ew.language.phrase("NoAddRecord"),
          returnFocus: false
        }); // Do not return focus
        this.tryFocus(true);
        return false;
    ...
    if ($__default.default.isObject(options.ajax)) {
      ...
      options.ajax = {
        ...
        transport: (params, success, failure) => $__default.default.ajax(params).then(data => {
          ...
          !error || _alert({title: error, icon: "error"});
          success(data);
        }).fail(failure)
      };
    }
  ...
  function sanitizeUrl(url) {
	...
	function setSessionTimer() {
		if (typeof ew.IS_LOGGEDIN == "undefined" || !ew.IS_LOGGEDIN)
		 	return; // Skip if not logged in
		var keepAliveTimer, timeoutTime, timeoutTime4, timer1, timer2, timer3, timer4, timer5, counter, counter2, $dlg = $__default['default']("div.ew-footer-text"),
			useKeepAlive = ((ew.SESSION_KEEP_ALIVE_INTERVAL > 0 || ew.IS_LOGGEDIN && ew.IS_AUTOLOGIN) && ew.TIMEOUT_URL);
		// Keep alive
		...
		// Reset timer
		var timeout4 = function() {
		if (timer4)
			timer4.cancel(); // Clear timer4	
			timer5 = $__default['default'].later(1000, null, function() {
				var message = '<span class="text-danger">' + ew.language.phrase("SessionCountDown") + '</span>';
				if (counter2 > 0) {
					if (counter2 <= ew.SESSION_TIMEOUT_COUNTDOWN || timeoutTime <= 0) {
						$dlg.find(".msTimer").html('');
						counter2 = 0;
					} else {
						//$dlg.find(".msTimer").html(ew.language.phrase("SessionCountDown").replace("%s", counter2));
						var sMinutes = "";
						var sSeconds = "";
						var minutes = Math.floor(counter2 / 60);
						...
						if (message.includes("%m") && message.includes("%s")) {
						  message = message.replace("%m", '<span class="ew-session-counter-minute">' + sMinutes + '</span>');
						  message = message.replace("%s", '<span class="ew-session-counter-second">' + sSeconds + '</span>');
						} else if (message.includes("%s")) {
						  message = message.replace("%s", '<span class="ew-session-counter-second">' + sSeconds + '</span>');
						}
						$dlg.find(".msTimer").html(message);
					}
				} else {
					$dlg.find(".msTimer").html('');
					$__default['default'].later(1000, null, function() {
						// do nothing
					});
				}
				counter2--;				
		...
		var resetTimer4 = function() {
			if (timeoutTime4 < 0) { // Timeout now
				timeoutTime4 = 0;
				counter2 = 0;
				timer5.cancel();
			}
		...
		var resetTimer = function() {
			...
			counter2 = ew.SESSION_TIMEOUT;
			timeoutTime4 = ew.SESSION_TIMEOUT;
		...
		var timeout = function() {
			...
			timer2 = $__default['default'].later(1000, null, function() { // Create a timer that runs every second
			if (counter > 0) {
				let timerInterval;
				var sMinutes = "";
				var sSeconds = "";
				var message = '<span class="text-danger">' + ew.language.phrase("SessionWillExpire") + '</span>';		
				...
				//sMessage = '<span class="text-danger">' + message + '</span>';
				// myString.replace(/<[^>]*>?/gm, '');
				document.title = message.replace(/<[^>]*>?/gm, ''); // ew.language.phrase("SessionWillExpire").replace("%s", counter);
				
				sMessage = '<div class="alert alert-dismissible ew-warning text-danger"><div class="row"><div class="col-2"><span style="font-size: 3rem;"><i class="icon fa-solid fa-triangle-exclamation" style="vertical-align: text-top;"></i></span></div><div class="col-10">&nbsp;' + message + '</div></div></div>';
				
				//sMessage = '<div class="alert alert-warning d-flex align-items-center" role="alert"><i class="fa-solid fa-circle-exclamation"></i><div>' + message + '</div></div>';
				alertify.alert(sMessage).set('onok', function(){ 
					if (timer2)
						timer2.cancel(); // Clear timer2
					keepAlive();
					...
					if (counter > 0) {
						...
						alertify.success(ew.language.phrase('MessageSessionExtended')).dismissOthers();
					}
				}).set('title', ew.language.phrase("MessageAlert")).set('modal', true); 
			} else { // Counter = 0, log out
				var sMessage = "";
				var message = "";
				message = ew.language.phrase("SessionExpired");
				sMessage = '<p class="text-danger" style="padding: 20px;">' + message + '</p>';
				alertify.alert(sMessage).set('title', ew.language.phrase("AlertifyAlert")).set('modal', true);
				$dlg.find(".msTimer").html(ew.language.phrase("SessionExpired"));
				resetTimer();
				timer4.cancel();
				timer5.cancel();
				$__default['default'].later(1000, null, function() {
					alertify.success(ew.language.phrase('MessageProcessing')).dismissOthers();
		...
		var setTimer = function() {
			...
			timer1 = $__default['default'].later(timeoutTime * 1000, null, timeout);
		};
		var setTimer4 = function() {
			resetTimer4(); // Reset timer first
			timer4 = $__default['default'].later(1000, null, timeout4);
		};
		...
		if (useKeepAlive) { // Keep alive
			...
			timer3 = $__default['default'].later(keepAliveInterval * 1000, null, keepAlive, null, true); // Periodic
  ...
  function fileDownload(href, data) {
    ...
    // Begin of Export Data moved from server-side to this client-side, by Masino Sinaga, September 27, 2022
	data = mergeSearchParams(data, [[ew.TOKEN_NAME_KEY, ew.TOKEN_NAME], [ew.ANTIFORGERY_TOKEN_KEY, ew.ANTIFORGERY_TOKEN]]);
	// End of Export Data moved from server-side to this client-side, by Masino Sinaga, September 27, 2022

    return swal.fire({
      ...ew.sweetAlertSettings,
      showConfirmButton: false,
      title: "<p>" + ew.language.phrase("Exporting") + "</p>",
        ...
        _fetch(href, {
          ...
          if (ct != null && ct.includes("json")) {
            ...
            if (error) {
              swal.hideLoading();
              swal.update({
                title: "<div class='text-danger'>" + error + "</div>",
  ...
  function _prompt(cfg, cb) {
    ...
    config = $__default.default.extend(true, {}, ew.sweetAlertSettings, {
      title: $__default.default.isString(cfg) ? cfg : "",
	  icon: "question",
  ...
  function _updateOptions(obj, parentId, async, change) {
    ...
    if (!$__default.default.isBoolean(async) || Array.isArray(list.lookupOptions) && list.lookupOptions.length > 0) {
      ...
      return $__default.default.ajax(getApiUrl(ew.API_LOOKUP_ACTION), {
        ...
        error: (jqXHR, textStatus, errorThrown) => _alert({title: errorThrown, icon: "error"}),
  ...
  function submitAction(e, args) {
    ...
    if ((isMultiple || isPostBack) && !f) {
      _alert({title: ew.language.phrase("NoHtmlForm"), icon: "error"});
      return false;
    }
    if (isMultiple && !keySelected(f)) {
      _alert({title: ew.language.phrase("NoRecordSelected"), icon: "error"});
      return false;
    }
  ...
  async function _export(args) {
    ...
    if (exportSelected && !keySelected(f)) {
      _alert({title: ew.language.phrase("NoRecordSelected"), icon: "warning"});
      return false;
    }
  ...
  async function inlineAction(args) {
    ...
    let _fail = function (o) {
      let msg_desc = '';
	  if (o.status == '404' || o.status == '403') {
	    msg_desc = ew.language.phrase("MessageError404Additional");
	  }
      !o.status || showToast("Server Error " + o.status + ": " + o.statusText + "<br>" + msg_desc);
    };
    ...
    if (ewAction == "inline") {
      if (action == "cancel") {
        ...
        _prompt({title: ew.language.phrase("DeleteConfirm"), icon: 'question'}, result => {
          if (result) {
            document.body.style.cursor = "wait";
  ...
  // Get form elements as object, property is HTML element or array of HTML element (deprecated), from v11, mod by Masino Sinaga, August 18, 2015
  function elementsToRow(fobj) {
    var $fobj = $__default.default(fobj), infix = $fobj.data("rowindex");
    infix = $__default.default.isValue(infix) ? infix : "";
    var	row = { "index": infix };
    $fobj.find("[name^=x" + infix + "_]").each(function() {
      ...
      if ($__default.default.isObject(row[elname])) { // Already exists
        if ($__default.default.isArray(row[elname])) {
    ...
    if (sameText(ew.TWO_FACTOR_AUTHENTICATION_TYPE, "google")) {
      ...
      $__default.default.get(url, result => {
        ...
        if (result != null && result.url) {
          _prompt({
            imageUrl: result.url,
            title: ew.language.phrase("Scan2FAQrCode"),
            input: "text",
          ...
          _alert({title: result.error.description, icon: "error"});
        }
        ...
        if (result.success) {
          _prompt({
            ...
            if (result != null && (_result$error5 = result.error) != null && _result$error5.description) {
              ...
              // Verify OTP
              _prompt({
                title: ew.language.phrase("EnterSecurityCode"),
                input: "text",
                confirmButtonText: ew.language.phrase("Verify"),
                showLoaderOnConfirm: true,
                allowEscapeKey: false,
  ...
  function disable2FA() {
    _prompt({
      title: ew.language.phrase("Disable2FAConfirm"),
      preConfirm: value => {
  ...
  function sendOTP(account) {
    _prompt({
      title: ew.language.phrase("SendOTPConfirm"),
      preConfirm: value => {
  ...
  function addOptionDialogShow(args) {
    ...
    if (!$dlg[0]) {
      _alert({title: "DIV #ew-add-opt-dialog not found.", icon: "error"});
      return;
    }
    ...
    var _submitSuccess = function (data) {
      ...
      if ((_results = results) != null && _results.success && results[objName]) {
        ...
        if ((_results2 = results) != null && _results2.error) {
          ...
          if (error) _alert({title: error, icon: "error"});
        } else {
          var msg,
    ...
    var _fail = function (o) {
      ...
      _alert({title: "Server Error " + o.status + ": " + o.statusText, html: msg_desc, icon: "error"});
    }; 
  ...
  function modalDialogShow(args) {
	...
	// Ask for dialog confirmation, by Masino Sinaga, October 5, 2021
	let _ask = function () {
      let args = $dlg.data("args");
      if (args.ask === null) return "";else if (args.ask && args.ask != "") return args.ask;
    };
    ...
    let _fail = function (o) {
      ...
      if (o.status) _alert({title: "Server Error " + o.status + ": " + o.statusText, html: msg_desc, icon: "error"});
    };
    ...
    let _submit = async function (e) {
      ...
      if (btn.classList.contains("ew-cancel")) {
		...
		_always(); // cursor normal
		// Begin of modification prompt before save in modal dialog, Masino Sinaga, September 26, 2021
		let $bd = $dlg.find(".modal-body");
        let cf = $bd.find("#confirm");		  
		//alert("action: " + args.action + "\n confirm: " + cf.val() + "\n ask: " + args.ask);
		if ( ((args.ask == '1' || _ask() == '1') && cf && cf.val() != "") || ($__default.default.isUndefined(cf) && (args.ask == '1' || _ask() == '1') && (args.action == "update" || args.action == "insert" || args.action == "edit") ) ) {
		  $(btn).prop("disabled", false).removeClass("disabled");
		  ew.prompt({title: ew.language.phrase("MessageConfirmSaveMessage"), showCancelButton: true, icon: 'question'}, result => {
			if (result) {
	    ...
	    // end of frm.canSubmit()
	    // End of modification prompt before save in modal dialog, Masino Sinaga, September 26, 2021
    ...
    let success = function (data) {
      ...
      if ($__default.default.isObject(result)) {
		...
		// Begin of modification Help Online in Modal dialog, by Masino Sinaga, October 21, 2019
		...
		var help_online_url = "";
		if (ew.MS_SHOW_HELP_ONLINE == 1) {
			help_online_url = "&nbsp;<a href='javascript:void(0);' id='helponline' onclick='getHelp(\""+ current_page +"\");' title='" + ew.language.phrase("Help") + "'><span class='fa fa-question-circle ew-icon'></span></a>";
		}
		$dlg.find(".modal-title").html(_caption() + help_online_url);
		// End of modification Help Online in Modal dialog, by Masino Sinaga, October 21, 2019
  ...
  function importDialogShow(args) {
    ...
    if (!$dlg[0]) {
      _alert({title: "DIV #ew-import-dialog not found.", icon: "error"});
      return false;
    }
  ...
  function emailDialogShow(args) {
    ...
    if (!$dlg[0]) {
      _alert({title: "DIV #ew-email-dialog not found.", icon: "error"});
      return false;
    }
    ...
    return swal.fire({
      ...
      willOpen: () => {
        ...
        _fetch(getApiUrl([ew.API_EXPORT_ACTION, "email", tblVar]) + "?rnd=" + random(), {
          ...
          if (result != null && result.success) {
            swal.update({
              title: "<div class='text-success'>" + result.message + "</div>"
            });
            $document$1.trigger("export", [{
              type: "done",
              tblVar,
            ...
            var _ref3;
            swal.update({
              title: (_ref3 = "<div class='text-danger'>" + getError(result)) != null ? _ref3 : ew.language.phrase("FailedToExport") + "</div>"
            });
            $document$1.trigger("export", [{
              type: "fail",
              tblVar,
          ...
          swal.hideLoading();
          swal.update({
            title: (_ref4 = "<div class='text-danger'>" + error) != null ? _ref4 : ew.language.phrase("FailedToExport") + "</div>"
          });
          $document$1.trigger("export", [{
            type: "fail",
            tblVar,
  ...
  function toggleLayout(el) {
    ...
    }).catch(error => _alert({title: error, icon: "error"})).finally(() => document.body.style.cursor = "default");
  }
  ...
  function showMessage(arg) {
    ...
    if ($div.length) {
      ...
      if (!ew.MS_USE_MESSAGE_BOX_INSTEAD_OF_TOAST) {
		...
		// This is needed, to display msgbox for Custom Action in List Page specially in Users List Page!
	  ...
	  } else if (ew.MS_USE_MESSAGE_BOX_INSTEAD_OF_TOAST) {
		let $msg_type = "";
		$.each(["success", "info", "warning", "danger"], function(index, value) {
			$div.find(".alert-" + value).toggleClass("alert-" + value + " text-" + value);
		});
		var html = $__default.default.isString(arg) ? arg : ($div.length ? (ew.IS_MOBILE ? $div.text() : $div.html()) : "");
		if (html.toLowerCase().includes("ew-success"))
			$msg_type = $msg_type + "success";
		if (html.toLowerCase().includes("ew-warning"))
			$msg_type = $msg_type + ",warning";
		if (html.toLowerCase().includes("ew-error"))
			$msg_type = $msg_type + ",error";
		if (html.toLowerCase().includes("ew-info"))
			$msg_type = $msg_type + ",info";
		
		if (html.trim() == "")
			return;
		if ($msg_type != "success") {
		  alertify.alert(html, function (ok) { }).set('title', ew.language.phrase("MessageAlert")).set('modal', true); // Modification Alertify by Masino Sinaga, October 14, 2013, last modified: May 20, 2015
		} else if ($msg_type == "success") { // auto-close dialog for success message only!
		  if(!alertify.successalert){
			  alertify.dialog('successalert',function factory(){
				return{
				  main:function(message){
					this.message = '<p class="text-danger">' + message + '</p>';
				  },
				  setup:function(){
					  return { 
						buttons:[{
		  ...
		  alertify.successalert(html).set('title', ew.language.phrase("MessageAlert")).set('modal', true).set('maximizable', false).autoOk(5); // Modification Alertify by Masino Sinaga, October 14, 2013, last modified: October 4, 2022
		}
  ...
  function upload(input) {
    ...
    let _change = function (e, data) {
      ...
      if ($__default.default.isNumber(cnt) && cnt > 0 && ar.length > cnt) {
        _alert({title: ew.language.phrase("UploadErrorMaxNumberOfFiles"), icon: "error"});
        return false;
      }
      let l = $input.data("size");
      if ($__default.default.isNumber(l) && l > 0 && ar.join(ew.MULTIPLE_UPLOAD_SEPARATOR).length > l) {
        _alert({title: ew.language.phrase("UploadErrorMaxFileLength"), icon: "error"});
        return false;
      }
    };
   ...
   * Format my Date Time
   *
   * @param {Date} my_date - JS start date/time
   * @returns {string}
   */

  function formatMyDateTime(my_date) {
    let from = DateTime.fromJSDate(my_date, {
      zone: "utc"
    }); 
    // Not all day => Show date/time
    if (from.isValid) return from.startOf("day") ? formatDateTime(from, ew.DATE_FORMAT + " " + ew.TIME_FORMAT) : formatDateTime(from, ew.TIME_FORMAT);else if (from.isValid) return formatDateTime(from, ew.TIME_FORMAT);
    return "";
  }
  ...
  function initPage(e) {
    ...
    // Show message
    if ( (typeof ew.USE_JAVASCRIPT_MESSAGE != "undefined" && ew.USE_JAVASCRIPT_MESSAGE) || (typeof ew.MS_USE_MESSAGE_BOX_INSTEAD_OF_TOAST != "undefined" && ew.MS_USE_MESSAGE_BOX_INSTEAD_OF_TOAST) ) showMessage(e);
  }
  ...
  function redirect(url, f, method, data) {
    ...
    if (url.startsWith("http")) {
      ...
      _alert({title: ew.language.phrase("IncorrectUrl"), icon: "error"});
      return false;
    }
  ...
  function fullCalendar(el, options, dropdown) {
    ...
    let processJson = data => {
      ...
      if (data.success) {
		...
		ew.alert({title: ew.language.phrase("MessageUpdated"), icon: "success"});
      }
    };
    ...
    // Delete event
    let deleteEvent = id => ew.prompt({title: ew.language.phrase("DeleteEventConfirm"), icon: 'question'}, result => {
    ...
    let getOrCreatePopover = info => {
	  ...
	  info.event.setExtendedProp("dateStart", ew.formatMyDateTime(info.event.start, ew.DATE_FORMAT + " " + ew.TIME_FORMAT));
      info.event.setExtendedProp("dateEnd", ew.formatMyDateTime(info.event.end, ew.DATE_FORMAT + " " + ew.TIME_FORMAT));
	  info.event.setExtendedProp("descText", info.event.description);
    ...
    let opts = ew.deepAssign(fullCalendarOptions, {
      ...
      select: event => {
		Cookies.set('calendarDefaultDate', ew.formatDateTime(event.start, "yyyy-MM-dd"), {expires:365, path: '/'});
		Cookies.set('calendarDefaultView', event.view.type, {expires:365, path: '/'});
		addUrl && addEventDialogShow(event);
	  },
      ...
      eventClick: info => {
		...
		Cookies.set('calendarDefaultDate', ew.formatDateTime(info.event.start, "yyyy-MM-dd"), {expires:365, path: '/'});
		Cookies.set('calendarDefaultView', info.view.type, {expires:365, path: '/'});
      },
