...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fstats_counterlogsrch" id="fstats_counterlogsrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="on">
<div id="fstats_counterlogsrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { stats_counterlog: currentTable } });
var currentForm;
var fstats_counterlogsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="fstats_counterlogsrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="fstats_counterlogsrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="fstats_counterlogsrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="fstats_counterlogsrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_stats_counterlog" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_stats_counterloglist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Page->IP_Address->Visible) { // IP_Address ?>
        <th data-name="IP_Address" class="<?= $Page->IP_Address->headerCellClass() ?>"><div id="elh_stats_counterlog_IP_Address" class="stats_counterlog_IP_Address"><?= $Page->renderFieldHeader($Page->IP_Address) ?></div></th>
<?php } ?>
<?php if ($Page->Hostname->Visible) { // Hostname ?>
        <th data-name="Hostname" class="<?= $Page->Hostname->headerCellClass() ?>"><div id="elh_stats_counterlog_Hostname" class="stats_counterlog_Hostname"><?= $Page->renderFieldHeader($Page->Hostname) ?></div></th>
<?php } ?>
<?php if ($Page->First_Visit->Visible) { // First_Visit ?>
        <th data-name="First_Visit" class="<?= $Page->First_Visit->headerCellClass() ?>"><div id="elh_stats_counterlog_First_Visit" class="stats_counterlog_First_Visit"><?= $Page->renderFieldHeader($Page->First_Visit) ?></div></th>
<?php } ?>
<?php if ($Page->Last_Visit->Visible) { // Last_Visit ?>
        <th data-name="Last_Visit" class="<?= $Page->Last_Visit->headerCellClass() ?>"><div id="elh_stats_counterlog_Last_Visit" class="stats_counterlog_Last_Visit"><?= $Page->renderFieldHeader($Page->Last_Visit) ?></div></th>
<?php } ?>
<?php if ($Page->Counter->Visible) { // Counter ?>
        <th data-name="Counter" class="<?= $Page->Counter->headerCellClass() ?>"><div id="elh_stats_counterlog_Counter" class="stats_counterlog_Counter"><?= $Page->renderFieldHeader($Page->Counter) ?></div></th>
<?php } ?>
<?php
...
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){if(fstats_counterlogupdate.validateFields()){ew.prompt({title: ew.language.phrase("MessageEditConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_counterlogupdate").submit();});return false;} else { ew.prompt({title: ew.language.phrase("MessageInvalidForm"), icon: 'warning', showCancelButton:false}); }});});
</script>
<?php } ?>
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
...
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-grid-save, .ew-grid-insert').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_counterloglist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-inline-update').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_counterloglist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-inline-insert').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_counterloglist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){var gridchange=false;$('[data-table="stats_counterlog"]').change(function(){	gridchange=true;});$('.ew-grid-cancel,.ew-inline-cancel').click(function(){if (gridchange==true){ew.prompt({title: ew.language.phrase("ConfirmCancel"),icon:'question',showCancelButton:true},result=>{if(result) window.location = "<?php echo str_replace('_', '', 'stats_counterloglist'); ?>";});return false;}});});
</script>
<?php } ?>
<?php if (!$stats_counterlog->isExport()) { ?>
<script>
loadjs.ready("jqueryjs", function() {
	...
	SearchToggle.on('click', function(event) { 
		...
		if (SearchToggle.hasClass('active')) { 
			Cookies.set("stats_counterlog_searchpanel", "notactive", { path: '', expires: expires }); 
		} else { 
			Cookies.set("stats_counterlog_searchpanel", "active", { path: '', expires: expires }); 
		} 
	});
});
</script>
