...
<?php } ?>
<div id="gmp_stats_month" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_stats_monthlist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Page->Year->Visible) { // Year ?>
        <th data-name="Year" class="<?= $Page->Year->headerCellClass() ?>"><div id="elh_stats_month_Year" class="stats_month_Year"><?= $Page->renderFieldHeader($Page->Year) ?></div></th>
<?php } ?>
<?php if ($Page->Month->Visible) { // Month ?>
        <th data-name="Month" class="<?= $Page->Month->headerCellClass() ?>"><div id="elh_stats_month_Month" class="stats_month_Month"><?= $Page->renderFieldHeader($Page->Month) ?></div></th>
<?php } ?>
<?php if ($Page->Hits->Visible) { // Hits ?>
        <th data-name="Hits" class="<?= $Page->Hits->headerCellClass() ?>"><div id="elh_stats_month_Hits" class="stats_month_Hits"><?= $Page->renderFieldHeader($Page->Hits) ?></div></th>
<?php } ?>
<?php
...
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){if(fstats_monthedit.validateFields()){ew.prompt({title: ew.language.phrase("MessageEditConfirm"),icon:'question',showCancelButton:true},result=>{if(result)$("#fstats_monthedit").submit();});return false;} else { ew.prompt({title: ew.language.phrase("MessageInvalidForm"), icon: 'warning', showCancelButton:false}); }});});
</script>
<?php } ?>
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){if(fstats_monthupdate.validateFields()){ew.prompt({title: ew.language.phrase("MessageEditConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_monthupdate").submit();});return false;} else { ew.prompt({title: ew.language.phrase("MessageInvalidForm"), icon: 'warning', showCancelButton:false}); }});});
</script>
<?php } ?>
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
...
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-grid-save, .ew-grid-insert').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_monthlist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-inline-update').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_monthlist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-inline-insert').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fstats_monthlist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){var gridchange=false;$('[data-table="stats_month"]').change(function(){	gridchange=true;});$('.ew-grid-cancel,.ew-inline-cancel').click(function(){if (gridchange==true){ew.prompt({title: ew.language.phrase("ConfirmCancel"),icon:'question',showCancelButton:true},result=>{if(result) window.location = "<?php echo str_replace('_', '', 'stats_monthlist'); ?>";});return false;}});});
</script>
<?php } ?>
<?php if (!$stats_month->isExport()) { ?>
<script>
loadjs.ready("jqueryjs", function() {
	...
	SearchToggle.on('click', function(event) { 
		...
		if (SearchToggle.hasClass('active')) { 
			Cookies.set("stats_month_searchpanel", "notactive", { path: '', expires: expires }); 
		} else { 
			Cookies.set("stats_month_searchpanel", "active", { path: '', expires: expires }); 
		} 
	});
});
</script>
