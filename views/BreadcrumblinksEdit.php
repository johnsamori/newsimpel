...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="fbreadcrumblinksedit" id="fbreadcrumblinksedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { breadcrumblinks: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fbreadcrumblinksedit;
...
<?php } else { ?>
<table id="tbl_breadcrumblinksedit" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
...
<input type="hidden" data-table="breadcrumblinks" data-field="x_Page_Title" data-hidden="1" data-old name="o_Page_Title" id="o_Page_Title" value="<?= HtmlEncode($Page->Page_Title->OldValue ?? $Page->Page_Title->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } else { ?>
...
loadjs.ready(["fbreadcrumblinksedit", "jquerynumber"], function() {
	ew.createjQueryNumber("fbreadcrumblinksedit", "x_Lft", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
</span>
</div></div>
...
<script type="text/javascript">
loadjs.ready(["fbreadcrumblinksedit", "jquerynumber"], function() {
	ew.createjQueryNumber("fbreadcrumblinksedit", "x_Rgt", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fbreadcrumblinksedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fbreadcrumblinksedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
