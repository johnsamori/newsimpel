...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="fhelpedit" id="fhelpedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php } else { ?>
<table id="tbl_helpedit" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
...
<?php } else { ?>
    <tr id="r_Help_ID"<?= $Page->Help_ID->rowAttributes() ?>>
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_help_Help_ID"><?= $Page->Help_ID->caption() ?><?= $Page->Help_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></span></td>
...
loadjs.ready(["fhelpedit", "jquerynumber"], function() {
	ew.createjQueryNumber("fhelpedit", "x_Order", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
</span>
</div></div>
...
loadjs.ready(["fhelpedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fhelpedit", "x_Last_Updated", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
