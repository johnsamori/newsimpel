...
// Page object
$ProposalPengabdianStatusEdit = &$Page;
?>
<script>
...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="fproposal_pengabdian_statusedit" id="fproposal_pengabdian_statusedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { proposal_pengabdian_status: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fproposal_pengabdian_statusedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fproposal_pengabdian_statusedit")
        .setPageId("edit")
...
<?php } else { ?>
<table id="tbl_proposal_pengabdian_statusedit" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
<?php if ($Page->id_pengabdian->Visible) { // id_pengabdian ?>
<?php if ($Page->IsMobileOrModal) { ?>
    <div id="r_id_pengabdian"<?= $Page->id_pengabdian->rowAttributes() ?>>
        <label id="elh_proposal_pengabdian_status_id_pengabdian" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_pengabdian->caption() ?><?= $Page->id_pengabdian->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id_pengabdian->cellAttributes() ?>>
...
<input type="hidden" data-table="proposal_pengabdian_status" data-field="x_id_pengabdian" data-hidden="1" name="x_id_pengabdian" id="x_id_pengabdian" value="<?= HtmlEncode($Page->id_pengabdian->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } else { ?>
    <tr id="r_id_pengabdian"<?= $Page->id_pengabdian->rowAttributes() ?>>
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_proposal_pengabdian_status_id_pengabdian"><?= $Page->id_pengabdian->caption() ?><?= $Page->id_pengabdian->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></span></td>
...
<span id="el_proposal_pengabdian_status_Kelompok_ID">
    <select
        ...
        data-select2-id="fproposal_pengabdian_statusedit_x_Kelompok_ID"
        <?php } ?>
...
<?php if (!$Page->Kelompok_ID->IsNativeSelect) { ?>
<script>
loadjs.ready("fproposal_pengabdian_statusedit", function() {
    var options = { name: "x_Kelompok_ID", selectId: "fproposal_pengabdian_statusedit_x_Kelompok_ID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fproposal_pengabdian_statusedit.lists.Kelompok_ID?.lookupOptions.length) {
        options.data = { id: "x_Kelompok_ID", form: "fproposal_pengabdian_statusedit" };
    } else {
        options.ajax = { id: "x_Kelompok_ID", form: "fproposal_pengabdian_statusedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fproposal_pengabdian_statusedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fproposal_pengabdian_statusedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
