...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fkelompok_pengabdiansrch" id="fkelompok_pengabdiansrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="on">
<div id="fkelompok_pengabdiansrch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { kelompok_pengabdian: currentTable } });
var currentForm;
var fkelompok_pengabdiansrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready(["wrapper", "head"], function () {
    let $ = jQuery,
    ...
    let form = new ew.FormBuilder()
        .setId("fkelompok_pengabdiansrch")
        .setPageId("list")
    ...
    <div class="col-sm-auto px-0 pe-sm-2">
        <div class="ew-basic-search input-group">
            ...
            <div class="dropdown-menu dropdown-menu-end">
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "" ? " active" : "" ?>" form="fkelompok_pengabdiansrch" data-ew-action="search-type"><?= $Language->phrase("QuickSearchAuto") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "=" ? " active" : "" ?>" form="fkelompok_pengabdiansrch" data-ew-action="search-type" data-search-type="="><?= $Language->phrase("QuickSearchExact") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "AND" ? " active" : "" ?>" form="fkelompok_pengabdiansrch" data-ew-action="search-type" data-search-type="AND"><?= $Language->phrase("QuickSearchAll") ?></button>
                <button type="button" class="dropdown-item<?= $Page->BasicSearch->getType() == "OR" ? " active" : "" ?>" form="fkelompok_pengabdiansrch" data-ew-action="search-type" data-search-type="OR"><?= $Language->phrase("QuickSearchAny") ?></button>
            </div>
        </div>
    </div>
...
<?php } ?>
<div id="gmp_kelompok_pengabdian" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>" style="<?= $Page->TableContainerStyle ?>">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit() || $Page->isMultiEdit()) { ?>
<table id="tbl_kelompok_pengabdianlist" class="<?= $Page->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Page->Id_Kelompok->Visible) { // Id_Kelompok ?>
        <th data-name="Id_Kelompok" class="<?= $Page->Id_Kelompok->headerCellClass() ?>"><div id="elh_kelompok_pengabdian_Id_Kelompok" class="kelompok_pengabdian_Id_Kelompok"><?= $Page->renderFieldHeader($Page->Id_Kelompok) ?></div></th>
<?php } ?>
<?php if ($Page->Nama_Ketua->Visible) { // Nama_Ketua ?>
        <th data-name="Nama_Ketua" class="<?= $Page->Nama_Ketua->headerCellClass() ?>"><div id="elh_kelompok_pengabdian_Nama_Ketua" class="kelompok_pengabdian_Nama_Ketua"><?= $Page->renderFieldHeader($Page->Nama_Ketua) ?></div></th>
<?php } ?>
<?php if ($Page->Nama_Anggota_1->Visible) { // Nama_Anggota_1 ?>
        <th data-name="Nama_Anggota_1" class="<?= $Page->Nama_Anggota_1->headerCellClass() ?>"><div id="elh_kelompok_pengabdian_Nama_Anggota_1" class="kelompok_pengabdian_Nama_Anggota_1"><?= $Page->renderFieldHeader($Page->Nama_Anggota_1) ?></div></th>
<?php } ?>
<?php if ($Page->Nama_Anggota_2->Visible) { // Nama_Anggota_2 ?>
        <th data-name="Nama_Anggota_2" class="<?= $Page->Nama_Anggota_2->headerCellClass() ?>"><div id="elh_kelompok_pengabdian_Nama_Anggota_2" class="kelompok_pengabdian_Nama_Anggota_2"><?= $Page->renderFieldHeader($Page->Nama_Anggota_2) ?></div></th>
<?php } ?>
<?php if ($Page->Tanggal->Visible) { // Tanggal ?>
        <th data-name="Tanggal" class="<?= $Page->Tanggal->headerCellClass() ?>"><div id="elh_kelompok_pengabdian_Tanggal" class="kelompok_pengabdian_Tanggal"><?= $Page->renderFieldHeader($Page->Tanggal) ?></div></th>
<?php } ?>
<?php
...
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){if(fkelompok_pengabdianupdate.validateFields()){ew.prompt({title: ew.language.phrase("MessageEditConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fkelompok_pengabdianupdate").submit();});return false;} else { ew.prompt({title: ew.language.phrase("MessageInvalidForm"), icon: 'warning', showCancelButton:false}); }});});
</script>
<?php } ?>
<?php if (!$Page->IsModal && !$Page->isExport()) { ?>
<script>
...
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-grid-save, .ew-grid-insert').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fkelompok_pengabdianlist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-inline-update').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fkelompok_pengabdianlist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){$('.ew-inline-insert').on('click',function(){ew.prompt({title: ew.language.phrase("MessageSaveGridConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fkelompok_pengabdianlist").submit();});return false;});});
</script>
<script>
loadjs.ready(["wrapper", "head", "swal"],function(){var gridchange=false;$('[data-table="kelompok_pengabdian"]').change(function(){	gridchange=true;});$('.ew-grid-cancel,.ew-inline-cancel').click(function(){if (gridchange==true){ew.prompt({title: ew.language.phrase("ConfirmCancel"),icon:'question',showCancelButton:true},result=>{if(result) window.location = "<?php echo str_replace('_', '', 'kelompok_pengabdianlist'); ?>";});return false;}});});
</script>
<?php } ?>
<?php if (!$kelompok_pengabdian->isExport()) { ?>
<script>
loadjs.ready("jqueryjs", function() {
	...
	SearchToggle.on('click', function(event) { 
		...
		if (SearchToggle.hasClass('active')) { 
			Cookies.set("kelompok_pengabdian_searchpanel", "notactive", { path: '', expires: expires }); 
		} else { 
			Cookies.set("kelompok_pengabdian_searchpanel", "active", { path: '', expires: expires }); 
		} 
	});
});
</script>
