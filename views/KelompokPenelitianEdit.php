...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="fkelompok_penelitianedit" id="fkelompok_penelitianedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { kelompok_penelitian: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fkelompok_penelitianedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fkelompok_penelitianedit")
        .setPageId("edit")
...
<?php } else { ?>
<table id="tbl_kelompok_penelitianedit" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
...
<input type="hidden" data-table="kelompok_penelitian" data-field="x_Id_Kelompok" data-hidden="1" data-old name="o_Id_Kelompok" id="o_Id_Kelompok" value="<?= HtmlEncode($Page->Id_Kelompok->OldValue ?? $Page->Id_Kelompok->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } else { ?>
...
<span id="el_kelompok_penelitian_Nama_Ketua">
    <select
        ...
        data-select2-id="fkelompok_penelitianedit_x_Nama_Ketua"
        <?php } ?>
        data-table="kelompok_penelitian"
...
<?php if (!$Page->Nama_Ketua->IsNativeSelect) { ?>
<script>
loadjs.ready("fkelompok_penelitianedit", function() {
    var options = { name: "x_Nama_Ketua", selectId: "fkelompok_penelitianedit_x_Nama_Ketua" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fkelompok_penelitianedit.lists.Nama_Ketua?.lookupOptions.length) {
        options.data = { id: "x_Nama_Ketua", form: "fkelompok_penelitianedit" };
    } else {
        options.ajax = { id: "x_Nama_Ketua", form: "fkelompok_penelitianedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_kelompok_penelitian_Nama_Anggota_1">
    <select
        ...
        data-select2-id="fkelompok_penelitianedit_x_Nama_Anggota_1"
        <?php } ?>
        data-table="kelompok_penelitian"
...
<script>
loadjs.ready("fkelompok_penelitianedit", function() {
    var options = { name: "x_Nama_Anggota_1", selectId: "fkelompok_penelitianedit_x_Nama_Anggota_1" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fkelompok_penelitianedit.lists.Nama_Anggota_1?.lookupOptions.length) {
        options.data = { id: "x_Nama_Anggota_1", form: "fkelompok_penelitianedit" };
    } else {
        options.ajax = { id: "x_Nama_Anggota_1", form: "fkelompok_penelitianedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<span id="el_kelompok_penelitian_Nama_Anggota_2">
    <select
        ...
        data-select2-id="fkelompok_penelitianedit_x_Nama_Anggota_2"
        <?php } ?>
        data-table="kelompok_penelitian"
...
<script>
loadjs.ready("fkelompok_penelitianedit", function() {
    var options = { name: "x_Nama_Anggota_2", selectId: "fkelompok_penelitianedit_x_Nama_Anggota_2" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fkelompok_penelitianedit.lists.Nama_Anggota_2?.lookupOptions.length) {
        options.data = { id: "x_Nama_Anggota_2", form: "fkelompok_penelitianedit" };
    } else {
        options.ajax = { id: "x_Nama_Anggota_2", form: "fkelompok_penelitianedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fkelompok_penelitianedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fkelompok_penelitianedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fkelompok_penelitianedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script>
