...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="fstats_monthedit" id="fstats_monthedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { stats_month: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fstats_monthedit;
...
<?php } else { ?>
<table id="tbl_stats_monthedit" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
...
<input type="hidden" data-table="stats_month" data-field="x_Year" data-hidden="1" data-old name="o_Year" id="o_Year" value="<?= HtmlEncode($Page->Year->OldValue ?? $Page->Year->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } else { ?>
...
loadjs.ready(["fstats_monthedit", "jquerynumber"], function() {
	ew.createjQueryNumber("fstats_monthedit", "x_Month", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
<input type="hidden" data-table="stats_month" data-field="x_Month" data-hidden="1" data-old name="o_Month" id="o_Month" value="<?= HtmlEncode($Page->Month->OldValue ?? $Page->Month->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } else { ?>
...
<script type="text/javascript">
loadjs.ready(["fstats_monthedit", "jquerynumber"], function() {
	ew.createjQueryNumber("fstats_monthedit", "x_Hits", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstats_monthedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fstats_monthedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
