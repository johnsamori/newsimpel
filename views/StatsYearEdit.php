...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="fstats_yearedit" id="fstats_yearedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
...
<?php } else { ?>
<table id="tbl_stats_yearedit" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
...
<input type="hidden" data-table="stats_year" data-field="x_Year" data-hidden="1" data-old name="o_Year" id="o_Year" value="<?= HtmlEncode($Page->Year->OldValue ?? $Page->Year->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } else { ?>
...
loadjs.ready(["fstats_yearedit", "jquerynumber"], function() {
	ew.createjQueryNumber("fstats_yearedit", "x_Hits", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstats_yearedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fstats_yearedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
