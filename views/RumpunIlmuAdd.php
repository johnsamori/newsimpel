...
<div class="col-md-12">
  <div class="card shadow-sm">
    ...
    <div class="card-body">
<?php } ?>
<form name="frumpun_ilmuadd" id="frumpun_ilmuadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<?php } else { ?>
<table id="tbl_rumpun_ilmuadd" class="<?= $Page->TableClass ?>"><!-- table* -->
<?php } ?>
...
loadjs.ready(["frumpun_ilmuadd", "jquerynumber"], function() {
	ew.createjQueryNumber("frumpun_ilmuadd", "x_no", {"number": true, "decimals": 0, "dec_point": "<?php echo $DECIMAL_SEPARATOR ?>", "thousands_sep" : "<?php echo $GROUPING_SEPARATOR ?>"});
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="frumpun_ilmuadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="frumpun_ilmuadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
